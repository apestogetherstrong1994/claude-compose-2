...
The author is starting a new piece of writing from scratch. Based on their description and any project context provided, write an opening that sets the right tone and gets them started.

Their description: "${toolParams?.description}"

Follow the description closely — if it asks for a single sentence, write exactly one sentence. If it describes a full piece, write an opening paragraph (3-5 sentences). Match the scope and style requested.

The opening should:
- Hook the reader naturally
- Give the author momentum to continue writing
- Feel like a strong starting point they can build on

DO NOT output any structural tags like [SUGGESTION]. Do NOT include any headers, titles, labels, or markdown formatting (no "# Opening Paragraph" etc.). Just output the opening directly as plain prose text. Write in a natural, engaging style appropriate for the described piece.
${ctx}`;

    case "draft":
      return `${BASE_PREAMBLE}

The author liked a brainstorm idea and wants you to turn it into actual prose for their document. The brainstorm idea was:

"${toolParams?.idea}"

IMPORTANT: The idea above is a meta-suggestion ABOUT what to write — it is NOT prose itself. Your job is to write the ACTUAL prose that implements this idea within the context of the document. Write as if you are the author continuing their story/essay/piece.
...
type: draft
target: _new
reasoning: 1-2 sentences explaining how you implemented the idea
text: The actual prose implementing the idea (1-3 paragraphs). Write in the author's voice as part of the document — NOT as commentary or suggestion.
[/SUGGESTION]

Rules:
- Write ACTUAL PROSE for the document, not a description of what to write
- Match the author's voice, tone, and style precisely
- The text should read as a natural continuation or addition to the existing document
- For fiction: write narrative prose (dialogue, description, action)
- For non-fiction: write argumentative/expository prose
- Output the [SUGGESTION] block as raw text, never in code fences
${ctx}`;
...
Generate exactly 3 distinct ideas. Each should be substantively different, not just variations on the same theme. Be creative and unexpected.
