...
export function useAIAction({ paragraphs, voiceProfile, projectConfig, ghostLength = "sentence" }) {
          ...
          if (newSuggestions.length > 0) {
            setSuggestions(prev => [
              ...
              ...newSuggestions.map(s => ({
                ...s,
                id: `s_${suggestionId++}`,
                // Use actual paragraph ID from toolParams, not Claude's echoed target
                target: toolParams?.paragraphId || s.target,
                originalText: toolParams?.selectedText || "",
                status: "pending",
              })),
            ]);
  ...
  // Generate an opening paragraph as ghost text (no delay, used for "Start from a Spark")
  const fetchOpeningSuggestion = useCallback((paragraphId, description) => {
    ...
    // Immediately start streaming (no debounce delay)
    (async () => {
      let accumulated = "";
      ...
      const controller = await streamFromAPI("/api/compose", {
        ...
        onDone: () => {
          // Ghost text stays visible until Tab accept or typing
        },
        onError: () => {
          setGhostText(null);
          setGhostParagraphId(null);
